...
if geojson_data is None:
    st.error("Failed to load Landuse_Ward GeoJSON data.")
else:
    ...
    def update_tables(subdivision, landuse):
        ...
        # Debug: Check if landuse_data is populated
        st.write("Filtered Landuse Data:")
        st.dataframe(landuse_data)

        # Summarize Landuse table
        if landuse_data.empty:
            st.warning("No Landuse data available for the selected filters.")
        ...
        # Debug: Check if filtered_gdf is populated
        st.write("Filtered ForestFire Data:")
        st.dataframe(filtered_gdf)

        # Summarize ForestFire table
        if filtered_gdf.empty:
            st.warning("No ForestFire data available for the selected filters.")
