...
def update_tables(subdivision, landuse):
    ...
    # Summarize Management Regime table
    management_regime_df = pd.DataFrame([
        {
            'Regime': feature['properties']['regime'],
            'Area_hectare': feature['properties'].get('Area_hectare', 0)
        } for feature in management_regime_data['features']
    ])
    management_regime_summary = (
        management_regime_df.groupby('Regime')
        .agg(Area_hectare=('Area_hectare', 'sum'))
        .reset_index()
    )
    total_area = management_regime_summary['Area_hectare'].sum()
    management_regime_summary['% of Total'] = (management_regime_summary['Area_hectare'] / total_area * 100).round(2)
    management_regime_summary['% of Total'] = management_regime_summary['% of Total'].astype(str) + '%'

    return landuse_summary, forestfire_summary, management_regime_summary
...
if apply_filters:
    ...
    landuse_summary, forestfire_summary, management_regime_summary = update_tables(selected_subdivision, selected_landuse)
    ...
    st.write("Management Regime Summary Table")
    st.dataframe(management_regime_summary)
    ...
    st.download_button(
        label="Download Management Regime Summary Table",
        data=management_regime_summary.to_csv(index=False),
        file_name="management_regime_summary_table.csv",
        mime="text/csv"
    )
else:
    # Initial Map
    ...
    # Initial Tables
    st.subheader("Summary Tables")
    landuse_summary, forestfire_summary, management_regime_summary = update_tables('All', 'All')
